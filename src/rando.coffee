  ...
  @randomSprintName: ->
    ...
    underscore.each randomPattern, (type) =>
      ...
      while usedWords.indexOf(randomWord) > -1
        switch type
          when ADJECTIVE   then randomWord = @_randomWord(ADJECTIVES)
          when NOUN        then randomWord = @_randomWord(NOUNS)
          when VERB        then randomWord = @_randomWord(VERBS)
          when CONJUNCTION then randomWord = @_randomWord(CONJUNCTIONS)
